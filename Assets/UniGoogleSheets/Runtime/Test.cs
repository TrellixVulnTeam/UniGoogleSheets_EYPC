...
namespace Game.Data {
    public class Test
    {
        ...
        public static async void LoadFromGoogle()
        {
            SheetData sheetData = await UniGoogleSheets.SheetDataReader.GetSheetDataFromGoogleAsync(typeof(Test).Namespace, nameof(Test), SpreadSheetId); 
            FieldInfo[] fields = typeof(Test).GetFields(BindingFlags.Public | BindingFlags.Instance);
            int idx = 0;  
            for (int row = 0; row < sheetData.RowCount; row++)
            {
